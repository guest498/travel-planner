...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get('/api/soundtrack/:location', async (req, res) => {
    try {
      const location = req.params.location;

      // Create a prompt for generating location-specific music recommendations
      const aiPrompt = `Generate a musical atmosphere for ${location}. Include:
                       1. Traditional and modern music genres associated with this location
                       2. The cultural significance of music in this location
                       3. 5 specific song recommendations that capture the location's essence`;

      const response = await ai.chat(aiPrompt);

      // Parse the AI response to extract structured music recommendations
      const content = response.message.content;

      // Extract genres (assuming they're mentioned with keywords like "genres", "styles", "music")
      const genreMatch = content.match(/genres?|styles?|music:? ([^.]+)/i);
      const genres = genreMatch ?
        genreMatch[1].split(',').map(g => g.trim()) :
        ['Local Music', 'Traditional', 'Contemporary'];

      // Extract cultural context (usually a paragraph about music significance)
      const culturalMatch = content.match(/cultural|significance|tradition([^.]+\.)/i);
      const culturalContext = culturalMatch ?
        culturalMatch[1].trim() :
        'Music plays an important role in local culture and traditions.';

      // Generate sample recommendations based on the AI response
      const recommendations = [
        {
          title: "Traditional Melody",
          artist: "Local Ensemble",
          genre: genres[0],
          description: "A classic piece representing local heritage"
        },
        {
          title: "Modern Fusion",
        ...
        },
        {
          ...
          genre: genres[2],
          description: "Capturing the city's modern spirit"
        }
      ];

      // Return structured soundtrack data
      const soundtrackData = {
        genres,
        mood: "Culturally immersive",
        recommendations,
        culturalContext
      };

      res.json(soundtrackData);
    } catch (error: any) {
